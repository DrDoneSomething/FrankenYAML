...
function popup_select_js()
{
    ...
    if (!$selected_mode)
        $html .= "<tr><td style=\"$result_height\" class=\"animated_result_container start\" colspan=\"$result_colspan\" id=\"{$popup_return_id}_td_js_output\">&nbsp;</td></tr>";
    ...
    foreach ($buttons as $button) {
        ...
        $html .= "</td>";
        if ($show_result)
            $html .= '<td class="animated_result_container start"  style="width:200px";" id="' .
...
function js_cmnd_receive_reference()
{
    ...
    $fixed_string = construct_command_string($command_string);
    if($fixed_string != $command_string)
        js_cmnd_reference("insert",$return_id,$fixed_string);
    $ref = tasmota_has_reference($command_string, "tooltip");
    $full = $tooltip = "";
    $found = false;
    $parsed = array();
    if ($ref) {
        $tooltip = $ref;
        $parsed = tasmota_has_reference($command_string, "parsed");
    }
