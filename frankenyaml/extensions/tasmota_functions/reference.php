...
function tasmota_has_reference($cmnd, $return_type = "full", $alt_value = false)
{
    ...
    $with_value = array(
        ...
        "status",
        "mqttretry");
    $without_value = array("mqtthost","mqttuser","mqttpassword");
    $setoption = array("setoption");
    ...
    $setoption = array_fill_keys($setoption, "setoption");
    $with_value = array_fill_keys($with_value, "with_value");
    $search = array_merge($with_scrap_number, $setoption, $with_value, $without_value);

    $found = false;
    foreach ($search as $var => $mode) {
        if($mode == "scrap_number"||$mode=="setoption")
...
function reference_value_calc($mode, $value, $value_array = false)
{
    ...
    switch ($mode) {
        case "pulsetime":
            return pulsetime_calc($value_array);
        case "mqttretry":
